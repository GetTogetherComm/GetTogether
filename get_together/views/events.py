...
def events_list(request, *args, **kwargs):
    ...
    events = Event.objects.filter(
        attendees=request.user.profile,
...
def show_event(request, event_id, event_slug):
    ...
    context = {
        ...
        "attendee_list": Attendee.objects.filter(event=event).order_by(
            "-role", "-status"
        ),
        "attendee_count": Attendee.objects.filter(
            event=event, status=Attendee.YES
        ).count(),
        "presentation_list": event.presentations.filter(
            status=Presentation.ACCEPTED
        ).order_by("start_time"),
        "pending_presentations": event.presentations.filter(
...
def create_event_team_select(request):
    ...
    return render(
        request, "get_together/events/create_event_team_select.html", {"teams": teams}
    )
...
def sponsor_event(request, event_id):
    ...
    if request.user.is_anonymous:
        return JsonResponse(
            {
                "status": "ERROR",
                "message": _("You must be logged in manage event sponsors."),
            }
        )
    ...
    if not request.user.profile.can_edit_event(event):
        return JsonResponse(
            {
                "status": "ERROR",
                "message": _("You can not manage this event's sponsors."),
            }
        )
    ...
    if request.GET.get("action", None) == "add":
        if sponsor in event.sponsors.all():
            return JsonResponse(
                {"status": "ERROR", "message": _("Already sponsoring this event.")}
            )
    ...
    if request.GET.get("action", None) == "remove":
        if sponsor not in event.sponsors.all():
            return JsonResponse(
                {"status": "ERROR", "message": _("Not sponsoring this event.")}
            )
...
def attended_event(request, event_id):
    ...
    if request.user.is_anonymous:
        return JsonResponse(
            {
                "status": "ERROR",
                "message": _("You must be logged in mark an attendee's actual status."),
            }
        )
        ...
        return JsonResponse(
            {
                "status": "ERROR",
                "message": _(
                    "You can not set an attendee's actual status until the event is over"
                ),
            }
        )
    ...
    return JsonResponse(
        {"status": "OK", "attendee_id": attendee.id, "actual": attendee.actual_name}
    )
...
def send_comment_emails(comment):
    context = {"comment": comment, "site": Site.objects.get(id=1)}
    email_subject = "New comment on: %s" % comment.event.name
    email_body_text = render_to_string(
        "get_together/emails/events/event_comment.txt", context
    )
    email_body_html = render_to_string(
        "get_together/emails/events/event_comment.html", context
    )
    email_from = getattr(
        settings, "DEFAULT_FROM_EMAIL", "noreply@gettogether.community"
    )

    for attendee in comment.event.attendees.filter(
        user__account__is_email_confirmed=True
    ):
        success = send_mail(
            from_email=email_from,
            html_message=email_body_html,
...
def remove_event_photo(request, photo_id):
    ...
    if (
        not request.user.profile.can_edit_event(event)
        and photo.uploader != request.user.profile
    ):
        messages.add_message(
            request,
            messages.WARNING,
            message=_("You can not make changes to this photo."),
        )
        return redirect(event.get_absolute_url())
    ...
    if request.method == "GET":
        ...
        context = {"photo": photo, "event": event, "remove_form": form}
...
def add_place_to_series(request, series_id):
    ...
    if request.method == "GET":
        ...
        if form.is_valid:
            ...
            context = {"series": series, "place_form": form}
            return render(
                request, "get_together/places/add_place_to_series.html", context
            )
    else:
        return redirect("home")
...
def change_event_host(request, event_id):
    ...
    if not request.user.profile.can_edit_event(event):
        messages.add_message(
            request, messages.WARNING, message=_("You can not change this event's host")
        )
        return redirect(event.get_absolute_url())
    ...
    if request.method == "GET":
        ...
        form.fields["team"].queryset = Team.objects.filter(
            member__user=request.user.profile, member__role__gte=Member.MODERATOR
        ).order_by("name")

        context = {"event": event, "change_form": form}
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request,
                messages.SUCCESS,
                message=_("Your event host has been changed."),
            )
            return redirect(new_event.get_absolute_url())
        else:
            context = {"event": event, "change_form": form}
...
def cancel_event(request, event_id):
    ...
    if request.method == "GET":
        ...
        context = {"team": event.team, "event": event, "cancel_form": form}
...
def edit_series(request, series_id):
    ...
    if request.method == "GET":
        ...
        context = {"team": series.team, "series": series, "series_form": form}
        ...
        if form.is_valid:
            ...
            context = {"team": event.team, "series": series, "series_form": form}
            return render(request, "get_together/events/edit_series.html", context)
    else:
        return redirect("home")
...
def delete_series(request, series_id):
    ...
    if request.method == "GET":
        ...
        context = {"team": series.team, "series": series, "delete_form": form}
