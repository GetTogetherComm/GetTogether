...
class ViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.userProfile = mommy.make(UserProfile, user=self.testuser, send_notifications=True)

        self.userAccount = mommy.make(Account, user=self.testuser, is_email_confirmed=True)

        self.org = mommy.make(Organization, owner_profile=self.userProfile, name='Test Org', slug='test-org')
        self.team = mommy.make(Team, organization=self.org, name='Test team', slug='test_team', owner_profile=self.userProfile)
        self.event = mommy.make(Event, team=self.team, start_time=datetime.datetime.now() + datetime.timedelta(hours=1), end_time=datetime.datetime.now() + datetime.timedelta(hours=2))
    ...
    def test_invite_members(self): 
        member = mommy.make(Member, team=self.team, user=self.userProfile, role=Member.ADMIN)
    ...
    def test_change_member_role(self): 
        ...
        member = mommy.make(Member, team=self.team, user=self.userProfile, role=Member.ADMIN)
        self.check_view('/team/%s/+change_role/%s/' % (self.team.id, member.id), login=True, status=302)
...
#        self.check_view('/+create-place/', login=True)
    ...
    def test_user_confirm_email(self): 
        ...
        self.check_view('/profile/+confirm_email/%s' % confirmation.key, login=True, status=302)
    ...
    def test_accept_request_to_join_org(self): 
        teamrequest = mommy.make(OrgTeamRequest, organization=self.org, request_origin=OrgTeamRequest.TEAM, requested_by=self.userProfile)
        self.check_view('/org/+confirm_request/%s/' % teamrequest.request_key, login=True)


    def test_accept_invite_to_join_org(self): 
        member = mommy.make(Member, team=self.team, user=self.userProfile, role=Member.ADMIN)
        orgrequest = mommy.make(OrgTeamRequest, team=self.team, request_origin=OrgTeamRequest.ORG, requested_by=self.userProfile)
        self.check_view('/org/+confirm_request/%s/' % orgrequest.request_key, login=True)
    ...
    def test_show_common_event(self): 
        common = mommy.make(CommonEvent, organization=self.org, name='Test common event')
    ...
    def test_create_event_team_select(self): 
        ...
        member1 = mommy.make(Member, team=self.team, user=self.userProfile, role=Member.ADMIN)

        team2 = mommy.make(Team, organization=self.org, name='Test team 2', slug='test_team_2', owner_profile=self.userProfile)
        member2 = mommy.make(Member, team=team2, user=self.userProfile, role=Member.ADMIN)
    ...
    def test_sponsor_event(self): 
        ...
        self.check_view('/events/%s/+sponsor/?sponsor=%s' % (self.event.id, sponsor.id), login=True)
    ...
    def test_attend_event(self): 
        settings.CSRF_VERIFY_TOKEN = False
        self.check_view('/events/%s/+attend/?response=no' % self.event.id, login=True, status=302)
        self.check_view('/events/%s/+attend/?response=maybe' % self.event.id, login=True, status=302)
        self.check_view('/events/%s/+attend/?response=yes' % self.event.id, login=True, status=302)
    ...
    def test_schedule_event_talks(self): 
        speaker = mommy.make(Speaker, user=self.userProfile, title="Test Speaker")
        talk = mommy.make(Talk, speaker=speaker, title="Test Talk")
        presentation = mommy.make(Presentation, event=self.event, talk=talk, status=Presentation.PROPOSED)
