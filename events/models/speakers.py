...
class Speaker(models.Model):
    ...
    def headshot_url(self):
        if (
            ...
            and self.avatar.name is not None
            and self.avatar.name != ""
        ):
...
class Talk(models.Model):
    ...
    def future_presentations(self):
        return self.presentations.filter(
            status__gte=0, event__start_time__gt=timezone.now()
        )

    @property
    def past_presentations(self):
        return self.presentations.filter(
            status=1, event__start_time__lte=timezone.now()
        )
...
class Presentation(models.Model):
    ...
    event = models.ForeignKey(
        Event, related_name="presentations", on_delete=models.CASCADE
    )
    ...
    start_time = models.DateTimeField(
        verbose_name=_("Start Time"), db_index=True, null=True, blank=True
    )
